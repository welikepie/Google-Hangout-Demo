...
var undefined,
    ...
    catch(e){ slice = function (e) { var i, ret=[]; for (i=0; e[i]; i++) ret.push(e[i]); return ret; }; }
        ...
        if (arguments.length == 2) {
            return this.each(function(el) {
                if (el.tagName && el.tagName.toLowerCase() == 'input' && attribute == 'value') el.value = val;
                  ...
                  if (val === false || val === null || typeof val === 'undefined') el.removeAttribute(attribute);
...
*/
	to: function(location) {
		...
		xui.apply(window, Array.prototype.slice.call(arguments, 1)).html(location, this);
		return this;
	},
 ...
 *
 */
xui.el = function(html) {
	var e = wrap(html);
	e = xui(e.childNodes).each(function(el) { e.removeChild(el); }); clean(e);
	return e;
};
/**
	Event
